...
class Database:
    ...
    async def get_proxy_for_country(self, country_code: str) -> Optional[Dict[str, Any]]:
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow('''
                SELECT host, port, username, password, ping_ms
                FROM proxies
...
async def step2_spam_check(session_string: str, phone_number: str,
                            ...
                            block_bots: bool = True, proxy: Optional[Dict] = None, spam_check_required: bool = True,
            ...
            try:
                ...
                for idx, dialog in enumerate(dialogs):
                    ...
                    try:
                        ...
                        # Block bots
                        # Block bots and delete chat history
                        if block_bots and is_bot:
                            await client.block_user(chat_id)
                            await client.delete_history(chat_id)   # নতুন লাইন
                            blocked_bots += 1
                            operation_performed = True
                            logger.info(f"Blocked and deleted bot: {chat_title}")
                ...
                logger.info(f"Cleanup summary - Total dialogs: {len(dialogs)}, Bots found: {bot_count}, Bots blocked: {blocked_bots}")
...
async def process_job(job: Dict[str, Any], db: Database):
    ...
    if not step1_done:
        ...
        if step1_result.get('success'):
            ...
            elif step1_result.get('error') == 'PasswordHashInvalid':
